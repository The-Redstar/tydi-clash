...
instance (KnownNat n, n~n0+1) => Strb (PStreamX c l s Bool n d u e sealed) where
  getStrb PStream{valid=True,strb} = Just $ repeat strb
  getStrb _ = Nothing
  unsafeGetStrb PStream{strb} = repeat strb
  mkStrb    = head
instance Strb (PStreamX c l s (Vec n Bool) n d u e sealed) where
  ...
  mkStrb s = s
instance (KnownNat n, n~n0+1) => Strb (PStreamTransfer c l s Bool n d u e) where
  getStrb PStreamTransfer{strb} = repeat strb
  mkStrb    = head
